...
var vik = new function() {
	...
	/** Ghost mode. */
	this._ghost = false;
	/** Redirection ghost mode. */
	this._ghostRedirection = false;
	/** Force POST method. */
	 ...
	 *				- ghostRedirection (boolean)	Tell if redirected loadings should be in ghost mode (no addition in history). (default: false)
	...
	this.init = function(params) {
		...
		this.onDocumentReady(function() {
			...
			// init of forms
			vik._initFormNodes();
			// create the event before executing the "post" callback and sending the "vik:postLoad" event
	...
	this.load = function(input, params) {
		...
		if (typeof input === "string") {
			...
			if (!params.hasOwnProperty("ghost"))
				params.ghost = true;
			if (!params.hasOwnProperty("ghostRedirection"))
				params.ghostRedirection = true;
		} else {
			...
			var val = null;
			if (input.hasAttribute("data-vik-post-method")) {
				val = input.getAttribute("data-vik-post-method");
				if (val == "true")
					params.postMethod = true;
			}
			...
			if (input.hasAttribute("data-vik-ghost-redirection")) {
				val = input.getAttribute("data-vik-ghost-redirection");
				if (val == "true")
					params.ghostRedirection = true;
				else if (val == "false")
					params.ghostRedirection = false;
			}
	...
	this._initParams = function(params) {
		...
		this._ghostRedirection = (params.ghostRedirection === true) ? true : false;
		this._postMethod = (params.postMethod === true) ? true : false;
		...
		this._target = (params.hasOwnProperty("target") && params.target) ? params.target : "body";
		this._source = (params.hasOwnProperty("source") && params.source) ? params.source : "body";
		this._strategy = "replace";
	...
	this._initFormFields = function(formNode) {
		...
		var elemSel = "input:not([type='hidden']):not([onchange]), select:not([onchange]), textarea:not([onchange])";
		var inputs = [];
	...
	this._loadExec = function(url, initParams) {
		...
		if (params.postMethod === true && !params.hasOwnProperty(postData))
			params.postData = {};
		// fetch the page data
		this.fetchHttp(realUrl, params.strategy, !asRaw, params.postData, function(content, finalUrl) {
			// reset quitPageConfirm
			vik.setQuitPageConfirmation(false);
			// update page title
			if (!asRaw && (params.ghost !== true || params.ghostRedirection !== true) && params.title) {
			...
			if (asRaw) {
				...
				// extract <script> tags
				var scripts = srcNode.querySelectorAll("script:not([data-vik='false'])");
			...
			// manage ghostRedirection
			if (params.ghostRedirection !== true && finalUrl != realUrl) {
				vik._log("Add redirection URL to browser history.");
				vik._previousUrl = vik._lastUrl;
				vik._lastUrl = finalUrl;
				// history state
				var state = {
					"url": finalUrl
				};
	...
	this._getParams = function() {
		var params = {
			...
			ghost: this._ghost,
			ghostRedirection: this._ghostRedirection,
			postMethod: this._postMethod,
			target: this._target,
			source: this._source,
	...
	this._onpopstate = function(event) {
		...
		// fetch the page data
		vik.fetchHttp(state.url, strategy, asHTML, null, function(content, finalUrl) {
			// write the content in the right place
			if (!asHTML) {
				targetNode.innerHTML = content;
			} else {
	...
	this.fetchHttp = function(url, strategy, asHTML, postData, handler) {
		...
		xhr.setRequestHeader("x-requested-with", "Vik " + this._VERSION);
		if (strategy)
		...
		xhr.onreadystatechange = function() {
			...
			if (asHTML === true)
				handler(this.responseXML, finalUrl);
			else
				handler(this.response, finalUrl);
		};
		xhr.send(postData);
	};
	/**
