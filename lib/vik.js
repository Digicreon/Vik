...
var vik = new function() {
	...
	this.load = function(input, params) {
		...
		if (this._quitPageConfirm) {
			this._log("Ask confirmation to quit the page.");
			if (!confirm(this._quitPageConfirmationText)) {
	...
	this._initFormFields = function() {
		// selector for forms which "quit page confirmation" feature is not disabled
		var formSel =   "form"
		              + ":not([data-vik-quit-page-confirmation='off'])"
		              + ":not([data-vik-quit-page-confirmation='false'])"
		              + ":not([data-vik-quit-page-confirmation='no'])"
		              + ":not([data-vik-quit-page-confirmation='disable'])"
		              + ":not([data-vik-quit-page-confirmation='0'])";
		if (!this._manageQuitPageConfirmation) {
			// selector for forms which "quit page confirmation" feature is enabled
			formSel =   "form[data-vik-quit-page-confirmation='on'],"
			          + "form[data-vik-quit-page-confirmation='true'],"
			          + "form[data-vik-quit-page-confirmation='yes'],"
			          + "form[data-vik-quit-page-confirmation='enable'],"
			          + "form[data-vik-quit-page-confirmation='1']"
		}
		// get the list of elligible forms
		var elements = document.querySelectorAll(formSel);
		if (!elements || !elements.length)
			return;
		// get the input elements
		var elemSel = "input:not([onchange]), select:not([onchange])";
		var inputs = [];
		for (var i = 0; i < elements.length; ++i) {
			var inputs = elements[i].querySelectorAll(elemSel);
			// loop on the inputs, and add an 'onchange' attribute to them
			for (var j = 0; j < inputs.length; ++j) {
				inputs[j].setAttribute("onchange", "vik.setQuitPageConfirmation(true)");
			}
		}
	};
	/**
